...
exports.login = async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('users')
      .select('id, password_hash, primeiro_login')
      .eq('email', email)
      .single();
...
exports.searchUsers = async (req, res) => {
  ...
  if (!nome || typeof nome !== 'string') {
    return res.status(400).json({
      message: 'Parâmetro "nome" é obrigatório e deve ser uma string.',
    });
  }
  ...
  try {
    ...
    console.error('Erro no servidor ao buscar usuários:', err);
    res.status(500).json({ message: 'Erro no servidor.' });
  }
};
...
exports.addInteresse = async (req, res) => {
  ...
  try {
    const { error } = await supabase.from('interesses').insert([{ nome }]);
...
exports.addUsuarioGrupoEstudo = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: 'Usuário adicionado ao grupo de estudo com sucesso' });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: 'Erro no servidor' });
  }
  ...
  try {
    const { data, error } = await supabase
      ...
      .select(
        `
        amigo_id,
        users!conexoes_amigo_id_fkey (name)
      `,
      )
      .eq('usuario_id', id)
      .eq('status', 'aceito');
    ...
    // Transformar o retorno para facilitar o consumo no frontend
    const conexoes = data.map((conexao) => ({
      amigo_id: conexao.amigo_id,
      nome: conexao.users.name,
    }));
...
exports.addConexao = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      return res
        .status(500)
        .json({ message: 'Erro ao enviar solicitação de conexão' });
    }
...
exports.aceitarConexao = async (req, res) => {
  ...
  try {
    ...
    if (error) {
      return res
        .status(500)
        .json({ message: 'Erro ao aceitar solicitação de conexão' });
    }
...
exports.participarEvento = async (req, res) => {
  ...
  try {
    const { data: evento, error } = await supabase
      .from('eventos')
      .select(
        'limite_participantes, (SELECT COUNT(*) FROM evento_participantes WHERE evento_id = eventos.id) AS total_participantes',
      )
      .eq('id', evento_id)
      .single();
